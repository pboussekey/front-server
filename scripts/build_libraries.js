...
if( pkg.dependencies ){
    ...
    orderedLibraries.forEach( library => {
        ...
        // Check if content must be transpiled.
        if( pkg[opt] && pkg[opt][library] ){
            if( pkg[opt][library].src ){
                console.log('Set custom source for '+library+'...');
                finalPath =rootPath+'/node_modules/'+library+'/'+pkg[opt][library].src;
            }else if( pkg[opt][library].browserify ){
                let alias = undefined;
            ...
            }else if( pkg[opt][library].babel ){
                console.log('Babel '+library+'...');
        ...
        console.log('Append '+finalPath+' to lib.js...\n');
